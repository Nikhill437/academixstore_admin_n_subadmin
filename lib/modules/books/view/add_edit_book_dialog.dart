  ...
  Future<void> _saveBook() async {
    ...
    try {
      ...
      final bookData = {
        ...
        'year': _yearController.text.trim(), // Send as string (varchar)
        'semester': semester,
      ...
      if (bookId != null) {
        ...
        // Upload question paper PDF if selected (just like book PDF and cover)
        if (_qpPdfFileName != null &&
            (_qpPdfFilePath != null || _qpPdfFileBytes != null)) {
          print('🔥 Uploading question paper PDF: $_qpPdfFileName');
          Get.log('📝 Uploading question paper PDF: $_qpPdfFileName', 
            isError: false);
          
          final qpSuccess = await _booksController.uploadBookPdf(
            bookId,
            _qpPdfFilePath,
            fileBytes: _qpPdfFileBytes,
            fileName: _qpPdfFileName,
          );

          if (qpSuccess) {
            print('🔥 ✅ Question paper PDF uploaded successfully');
            Get.log('✅ Question paper PDF uploaded successfully', 
              isError: false);
          } else {
            print('🔥 ❌ Question paper PDF upload failed');
            Get.log('❌ Question paper PDF upload failed', 
              isError: true);
          }
        }
  ...
  Widget _buildQuestionPapersSection() {
    return Column(
      ...
      children: [
        ...
        // Question Paper PDF Upload (just like book PDF and cover)
        _buildFileUploadButton(
          label: 'Upload Question Paper PDF',
          icon: Icons.picture_as_pdf,
          fileName: _qpPdfFileName,
          onPressed: _pickQuestionPaperPdf,
          color: Colors.orange,
        ),
      ],
    );
